...
class MAPCalculator:
    # TODO: fix classes could be none - or should it not be?
    def __init__(self, min_overlap=0.5, classes=None, round_decimals=2):
    ...
    def calculate(self, print_=True):
        ...
        # for class_index, class_name in tqdm(enumerate(self.gt_classes), desc="class progress", disable=not print_, leave=True):
        for class_index, class_name in enumerate(self.gt_classes):
            ...
            # for idx, detection in tqdm(enumerate(dr_data), desc="class progress", disable=not print_, leave=True):
    ...
    # TODO: implement 'other' class?
    def add_gt_pred_pair(self, gts_single: np.ndarray, preds_single:np.ndarray, filter_classes: list=None,
                         group=False, group_other=False):
        if len(gts_single) == 0 and len(preds_single) == 0:
            return

        # TODO: fix rounding of confidence...
        if self.round_decimals is not None:
            if len(gts_single) > 0:
                # gts_single = torch.round(gts_single * 10 ** self.prune_decimals) / (10 ** self.prune_decimals)
                gts_single[:, 0:4] = np.round(gts_single[:, 0:4], decimals=self.round_decimals)
            if len(preds_single) > 0:
                preds_single[:, 0:4] = np.round(preds_single[:, 0:4], decimals=self.round_decimals)
                # preds_single = torch.round(preds_single * 10 ** self.prune_decimals) / (10 ** self.prune_decimals)

        # TODO:
        # test this wihtout grouping with different class sizes
        # return per class ap as well as map
        # test subset of problem (without grouping)
        # test with grouping - see if number of samples is the same as nongrouped?
        # test with other grouping - see if first grouped result is the same
        # compare with original implementation...

        # class filtering to be tested! TODO: thorough testing required!
        if filter_classes is not None:
            ...
            for flt in filter_classes:
                conditions_gt += np.isclose(gts_single[:, 5], flt, atol=0.25)
                conditions_pr += np.isclose(preds_single[:, 5], flt, atol=0.25)

            if group_other and not group:
                raise RuntimeError("operation of grouping other classes but not grouping classes of choice currently unsupported")

            # index 0: classes of choice, index 1: other
            if group_other:
                self.classes = ["classes of choice", "other"]
                gts_single[conditions_gt, 5] = 0.0
                preds_single[conditions_pr, 5] = 0.0
                gts_single[True ^ conditions_gt, 5] = 1.0
                preds_single[True ^ conditions_pr, 5] = 1.0
            else:
                gts_single = gts_single[conditions_gt]
                preds_single = preds_single[conditions_pr]

                if len(gts_single) == 0 and len(preds_single) == 0:
                    return

                if group:
                    self.classes = ["class of choice"]
                    gts_single[:, 5] = 0.0
                    preds_single[:, 5] = 0.0
