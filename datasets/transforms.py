...
# from config_coco import EPSILON
from config_bdd100k import EPSILON
...
# TODO: add shear to pipeline?
def random_transform_fn(img_size):
    return ComposeWithLabel([RandomHorizontalFlipWithLabel(p=0.5),
...
def _get_affine_matrix(center, angle, translate, scale, shear):
    ...
    n_sin_angle_p_shear = -np.sin(angle + shear)
    sin_angle = np.sin(angle)
    cos_angle_shear = np.cos(angle + shear)

    RSS_ = np.matrix([[cos_angle, n_sin_angle_p_shear, 0.],
                      [sin_angle, cos_angle_shear, 0.],
                      [0., 0., 1. / scale]])
...
# transform label (rotate + translate)
def _affine_transform_label(label, affine_matrix):
    # label: [x_position, y_position, width, height, probability, <one_hot>] * number of detected objects on picture
    # affine matrix: 3x3 (x2) operator

    # flatten(np.array(affine_matrix))

    bbox_xywh = np.matrix(label[..., :4].numpy())  # bbox position

    xy_lt = bbox_xywh[:, :2].copy()  # left     tops
    xy_rb = xy_lt + bbox_xywh[:, 2:4].copy()  # right bottoms

    # copy left top and just replace one part to translate it to right bottom
    xy_rt = bbox_xywh[:, :2].copy()  # right    tops
    xy_rt[:, 0] = xy_rb[:, 0].copy()

    # copy left top and just replace one part to translate it to left bottom
    xy_lb = bbox_xywh[:, :2].copy()
    ...
    xy_lt_first_column = np.array(xy_lt[:, 0])
    xy_lb_first_column = np.array(xy_lb[:, 0])
    # issue: https://github.com/numpy/numpy/issues/5894
    ...
    bbox_xywh = np.concatenate((x1, y1, x2 - x1, y2 - y1), axis=1)
    label[..., :4] = torch.from_numpy(bbox_xywh)
    return label
