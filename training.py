...
# from config_coco import NOOBJ_COEFF, COORD_COEFF, IGNORE_THRESH, ANCHORS, EPSILON
from config_bdd100k import NOOBJ_COEFF, COORD_COEFF, IGNORE_THRESH, ANCHORS, EPSILON
...
def run_training(model, optimizer, dataloader, device, img_size, n_epoch, every_n_batch, every_n_epoch, ckpt_dir):
    ...
    for epoch_i in range(n_epoch):
        ...
        for batch_i, (imgs, targets, target_lengths) in tqdm(enumerate(dataloader), desc="batch progress",
                                                             total=len(dataloader), leave=True):
            # if batch_i < 0:
            #     continue
            ...
            with torch.autograd.detect_anomaly():
                ...
                # imh = hash(str(imgs.to("cpu").numpy()))
                # lbh = hash(str(targets.to("cpu").numpy()))

                # print("imgs hash", imh)
                # print("labels hash", lbh)
                # print("lens", target_lengths)
                ...
                try:
                    ...
                    # get_gradients(model)
                    # print(get_max_gradients_per_layer(model))
            ...
            loss_sum += losses[0].item()

            if batch_i % 100 == 0:
                logging.info(
                    "[Epoch {}/{}, Batch {}/{}] [Losses: total {}, coord {}, obj {}, noobj {}, class {}]"
                        .format(epoch_i, n_epoch, batch_i, len(dataloader), losses[0].item(), losses[1].item(),
        ...
        loss_sum = loss_sum / (batch_i + 1)
        print("avg loss for epoch {}: {}".format(epoch_i, loss_sum))
